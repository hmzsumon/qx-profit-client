...
export default function StatCard({ label, value, tone = "neutral" }: Props) {
  ...
  return (
    <div className={`min-w-0 rounded-2xl border bg-gradient-to-br p-3 sm:p-4 ${toneClass}`}>
      <p className="text-xs text-white/55">{label}</p>
      <p className="mt-2 break-words text-base font-bold tabular-nums text-white sm:text-xl">{value}</p>
    </div>
  );
}
